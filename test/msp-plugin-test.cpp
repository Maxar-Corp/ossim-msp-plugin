...
#include "../src/plugin/ossimMspTool.h"
...
int runTest(ossimRefPtr<ossimTool> isaTool,  ossimArgumentParser& ap)
{
   ...
   {
      clog<<"ossim-isa-tool -- Error returned from ossimMsp tool initialization."<<endl;
      return 1;
   }
   if (!isaTool->execute())
   {
      clog<<"ossim-isa-tool -- Error returned from ossimMsp tool execute()."<<endl;
      return 1;
   }
   return 0;
}
...
{
   clog << "ossimMsp Plugin Test" << endl;
   ...
   isaTool = new ossimMsp::ossimMspTool;
   runTest(isaTool, ap2);
#endif
